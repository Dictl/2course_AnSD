...
std::vector<std::string> splitString(std::string str) {
    ...
    str.erase(std::remove(str.begin(), str.end(), ' '), str.end());//удаление пробелом из строки
    ...
    for (size_t i = 0; i < str.size(); ++i) {
        ...
        if ((c == 's' || c == 'c') && i+2 < str.size()) {//sin И cos
...
std::vector<std::string> toRPN(const std::string& expr) {
    ...
    std::unordered_map<std::string, int> precedences {//объявление приоритетности
            {"+", 1}, {"-", 1},
            {"*", 2}, {"/", 2},
            {"^", 3},
            {"sin", 4}, {"cos", 4}
...
double evaluate(const std::vector<std::string>& expr) {
    ...
    for (std::string token : expr) {
        if (isdigit(token[0])) {
            ...
            opStack.push(calc);
                std::cout << "Считаем " << op1 << token << op2 << ". \tSteck: " << opStack << std::endl;
        }
    }
    return std::stod(opStack.pop());
}// постепенный подсчёт и вывод в консоль
